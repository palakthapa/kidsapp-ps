...
export default function LearningModulesAdminScreen() {
    ...
    useEffect(() => {
        ...
        AxiosHelper("/api/admin/learning", "GET").then(function (response) {
    ...
    const editModalAcceptHandler = async (data) => {
        try {
            ...
            formData.append("includedIn", JSON.stringify(includedIn));
            if (imageRef.current.files) formData.append("imageFile", imageRef.current.files[0]);
            const { data } = await AxiosHelper("/api/admin/learning/" + moduleId, "PATCH", {}, formData);
            console.log(data);
    ...
    const openEditModalHandler = (data) => {
        ...
        setIncludedIn(data.included_in);
    ...
    const newModalAcceptHandler = async () => {
        try {
            ...
            const { data } = await AxiosHelper("/api/admin/learning", "POST", {}, formData);
            console.log(data);
        ...
        <Layout title="Admin Console">
            ...
            >
                ...
                <div className='w-full flex flex-col mt-5'>
                    ...
                    <div className='sm:w-4/5 mb-2'>
                        <label className='font-semibold sm:mr-2'>Module included in :</label>
                        <div className='w-full flex flex-col sm:flex-row justify-around'>
                            <div>
                                <input id='kb' type='checkbox' className='mr-2' checked={includedIn.keyboard} onChange={(e) => { setIncludedIn(state => ({ ...state, keyboard: e.target.checked })) }} />
                                <label htmlFor='kb'>Keyboard</label>
                            </div>
                            <div>
                                <input id='dnd' type='checkbox' className='mr-2' checked={includedIn.draganddrop} onChange={(e) => { setIncludedIn(state => ({ ...state, draganddrop: e.target.checked })) }} />
                                <label htmlFor='dnd'>Drag and Drop</label>
                            </div>
                            <div>
                                <input id='draw' type='checkbox' className='mr-2' checked={includedIn.draw} onChange={(e) => { setIncludedIn(state => ({ ...state, draw: e.target.checked })) }} />
                                <label htmlFor='draw'>Draw</label>
                            </div>
                        </div>
                    </div>
                    <div className='sm:w-4/5'>
                        <label className='font-semibold'>Module Image :</label>
                        <div className='flex flex-col items-center sm:mt-2'>
                            {imageUrl ? <img src={imageUrl} width={150} height={150} alt="Module cover image" className='h-auto mx-auto' /> : null}
                            <input type='file' name='moduleImage' ref={imageRef} accept="image/*" className='mt-2' onChange={imageChangeHandler} />
                        </div>
                    </div>
                </div>
            </Modal>
