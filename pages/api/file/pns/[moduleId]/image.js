...
export default async function getLearningModules(req, res) {
    try {
        ...
        switch (method) {
            case 'GET':
                const moduleData = await PnSModule.findOne({ module_id: moduleId }, { images: true });
                if (!moduleData) {
                    return res.status(404).send("File not found");
                }

                let index = parseInt(req.query.n);
                if (Number.isNaN(index) || index < 0 || index >= moduleData.images.length) n = 0;

                // Get the file object from the URL
                try {
                    const file = File.fromURL(moduleData.images[index]);
                    await file.loadAttributes();
                    file.download().pipe(res);
                } catch (error) {
                    fs.createReadStream("./pages/api/file/broken_image.png").pipe(res);
                }
