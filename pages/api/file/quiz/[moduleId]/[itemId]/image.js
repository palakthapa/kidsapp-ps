...
export default async function getLearningModules(req, res) {
    try {
        ...
        switch (method) {
            case 'GET':
                const itemData = await quizItemsModel[moduleItemsModelMap[moduleId]].findOne({ item_id: req.query.itemId }, { image_url: true });
                if (!itemData) {
                ...
                try {
                    const file = File.fromURL(itemData.image_url);
